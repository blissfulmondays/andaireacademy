 ...
 */
if ( ! class_exists( 'ArabesqueMikadoSidebar' ) ) {
	class ArabesqueMikadoSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'arabesque' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'arabesque-mikado-admin-sidebar-script', MIKADO_ROOT . '/framework/admin/assets/js/mkdf-ui/mkdf-sidebar.js' );
			wp_enqueue_style( 'arabesque-mikado-admin-sidebar', MIKADO_ROOT . '/framework/admin/assets/css/mkdf-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_html__( 'Enter Name of the new Widget Area', 'arabesque' ) . "' name='mkdf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_html__( 'Add Widget Area', 'arabesque' ) . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'arabesque_mikado_filter_custom_widget_args', $args );

			if ( is_array( $this->sidebars ) ) {
				foreach ( $this->sidebars as $sidebar ) {
					$args['name']  = $sidebar;
					$args['id']    = 'mkdf-custom-sidebar-' . sanitize_text_field( $sidebar );
